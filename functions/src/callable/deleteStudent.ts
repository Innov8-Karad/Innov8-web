 ...
 */
async function deleteCloudinaryAsset(publicId: string, resourceType: string = "image") {
 ...
 */
async function deleteDocRefs(refs: admin.firestore.DocumentReference[]) {
  const batchSize = 400;
...
export const deleteStudent = onCall(
  ...
  async (request) => {
    ...
    if (!studentId || typeof studentId !== "string") {
      throw new HttpsError(
        "invalid-argument",
        "The studentId argument must be a non-empty string."
      );
    }
    ...
    if (!userDocSnap.exists) {
      throw new HttpsError("not-found", "Student document not found in Firestore database.");
    }

    const studentDocData = userDocSnap.data() || {};
    const profilePhotoPublicId = studentDocData.profilePhotoPublicId as string | undefined;
    const batchId = studentDocData.batchId as string | undefined;
    ...
    try {
      ...
      if (authError.code !== "auth/user-not-found") {
        ...
        throw new HttpsError(
          "internal",
          `Firebase Authentication deletion failed: ${authError.message || authError}`
        );
      } else {
    ...
    const refsToDelete: admin.firestore.DocumentReference[] = [userDocRef];
    ...
    const feeDocsMap = new Map<string, admin.firestore.QueryDocumentSnapshot>();
    ...
    const progressDocsMap = new Map<string, admin.firestore.QueryDocumentSnapshot>();
    ...
    if (progressDirectSnap.exists) {
      progressDocsMap.set(progressDirectRef.id, progressDirectSnap as any);
    }
    ...
    if (legacyDirectSnap.exists) {
      progressDocsMap.set(legacyDirectRef.id, legacyDirectSnap as any);
    }
    ...
    try {
      ...
      for (const subDoc of submissionsSnap.docs) {
        ...
        const cloudinaryPublicId = subData.cloudinaryPublicId as string | undefined;
        const fileType = subData.fileType as string | undefined;
